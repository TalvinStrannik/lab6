    ...
    public void run() {
        try {
            ...
            while(true) {
                ...
                field.canMove(this);
                //
                if((x+speedX >= Rect.getX()) && (x+speedX <= Rect.getWidth()+Rect.getX()) && (y+speedY <= Rect.getY()))
                    if(y + speedY >= Rect.getY()-radius){
                        // Достигли верхней плоскотси прямоугольника, отскакиваемся вверх
                        speedY = -speedY;
                        y = Rect.getY()-radius;
                    }

                if((y + speedY >= Rect.getY()) && (y+ speedY <= Rect.getHeight()+Rect.getY()) && (x+speedX <= Rect.getX()))
                    if((x + speedX>= Rect.getX()-radius)){
                        // Достигли правой плоскотси прямоугольника, отскакиваемся влево
                        speedX = -speedX;
                        x = Rect.getX()-radius;
                    }

                if((x+speedX >= Rect.getX()) && (x+speedX <= Rect.getWidth()+Rect.getX()) && (y+speedY >= Rect.getY()+Rect.getHeight()))
                    if((y + speedY<= Rect.getY()+Rect.getHeight()+radius)){
                        // Достигли нижней плоскотси прямоугольника, отскакиваемся вниз
                        speedY = -speedY;
                        y = Rect.getY()+Rect.getHeight()+radius;
                    }

                if((y + speedY >= Rect.getY()) && (y+ speedY <= Rect.getHeight()+Rect.getY()) && x+speedX >= Rect.getX()+Rect.getWidth())
                    if((x + speedX<= Rect.getX()+Rect.getWidth()+radius)){
                        // Достигли левой плоскотси прямоугольника, отталкиваемся вправо
                        speedX = -speedX;
                        x = Rect.getX()+Rect.getWidth()+radius;
                    }
                //
                if (x + speedX <= radius) {
